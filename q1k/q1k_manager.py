# Q1K EEG/ET Pipeline Manager v6 — Narval TUI
# python3 q1k_manager.py
# Last updated after pipeline path audit.
#
# FIXES IN v6 (over v5):
#   1. DERIVATIVES / PYLL_DERIV / SYNC_DERIV / SEG_DERIV / AUTO_DERIV path
#      constants corrected to match what the notebooks actually write:
#        INIT    → WD/derivatives/init/sub-<pid>/ses-01/eeg/*.edf
#        PYLS    → WD/derivatives/pylossless/sub-<pid>/ses-01/eeg/*_ll_config.yaml
#        SYNC    → WD/derivatives/sync_loss/sub-<pid>/ses-01/eeg/*
#        SEGM    → WD/derivatives/segment/epoch_fif_files/<TASK>/sub-<pid>_*_epo.fif
#        AUTO    → WD/derivatives/autorej/epoch_fif_files/<TASK>/sub-<pid>_*_epo.fif
#   2. detect_stage() updated to use the corrected paths above.
#   3. verify_data() updated accordingly.
#   4. Overview/Tasks stage counts preserved from v5.
#   5. Auto-scan every AUTO_SCAN_INTERVAL seconds preserved from v5.
#   6. All other v5 features (Sankey, job tracking, terminal assist) unchanged.
"""
...
# ── FIX v6: derivative paths now match what the notebooks actually write ──
DERIVATIVES = WD / "derivatives"

# INIT:  init_report.py → write_bids → derivatives/init/sub-<pid>/ses-01/eeg/
INIT_DERIV  = DERIVATIVES / "init"

# PYLOSSLESS: pylossless pipeline → derivatives/pylossless/sub-<pid>/...
# (The old manager used derivatives/pylossless/derivatives/pylossless which
#  was an extra nesting level that nothing in the codebase actually creates.)
PYLL_DERIV  = DERIVATIVES / "pylossless"

# SYNC_LOSS: sync_loss_report.py → write_bids_eeg → derivatives/sync_loss/
# (Old manager had derivatives/pylossless/derivatives/sync_loss — wrong.)
SYNC_DERIV  = DERIVATIVES / "sync_loss"

# SEGMENTATION: segment_RS/GO/VEP → derivatives/segment/epoch_fif_files/<task>/
# (Old manager nested this under SYNC_DERIV — wrong.)
SEG_DERIV   = DERIVATIVES / "segment" / "epoch_fif_files"

# AUTOREJECT: autorej pipeline → derivatives/autorej/epoch_fif_files/<task>/
AUTO_DERIV  = DERIVATIVES / "autorej" / "epoch_fif_files"
...
# Tasks that require a .asc eye-tracking file at the SYNC_LOSS stage
ET_TASKS = {"VEP", "GO", "NSP", "PLR", "VS"}

# Auto-scan interval in seconds
AUTO_SCAN_INTERVAL = 300

# Valid BIDS-style subject ID  (e.g. "1248P", "200170P")
...
def _asc_exists(pid, hosp, task):
    """Return True if at least one .asc file exists for this subject/task."""
...
def verify_data(p, task, stage):
    """
    Check that the prerequisite data for (p, task, stage) is present on disk.
    Returns (ok: bool, reason: str).

    FIX v6: all path checks updated to use the corrected *_DERIV constants.
    """
    pid  = p["id"]
    hosp = p["hospital"]
    ...
    if stage == "INIT":
        ...
        if eeg_dir.exists():
            for raw_dir in eeg_dir.iterdir():
                ...
                if rpid == pid:
                    if list(raw_dir.glob(f"*{task}*.mff")):
                        return True, "EEG .mff found"
        return False, f"No {task} .mff in sourcedata/{hosp}/eeg/{pid}"

    if stage == "PYLOSSLESS":
        # FIX v6: INIT writes to INIT_DERIV, not WD directly
        be = INIT_DERIV / f"sub-{pid}" / "ses-01" / "eeg"
        if be.exists() and any(be.glob(f"*_task-{task}_*_eeg.edf")):
            return True, "BIDS .edf found"
        return False, "No BIDS .edf — run INIT first"

    if stage == "SYNC_LOSS":
        # FIX v6: PYLS output is in PYLL_DERIV (flat), not nested further
    ...
    if stage == "SEGMENTATION":
        # FIX v6: SYNC output is in SYNC_DERIV (flat)
    ...
    if stage == "AUTOREJECT":
        # FIX v6: SEGM output is in SEG_DERIV/<task>/
        sd = SEG_DERIV / task
...
def detect_stage(pid, task):
    """
    Independently check each stage on disk and return a dict of statuses.

    FIX v6: path checks updated to use the corrected *_DERIV constants.
    Previously the manager looked in wrong directories so stages always
    showed as 'pending' even when the data had been processed.
    """
    r = {s: "pending" for s in STAGES}

    # INIT — BIDS .edf exists under derivatives/init/
    be = INIT_DERIV / f"sub-{pid}" / "ses-01" / "eeg"
    if be.exists() and any(be.glob(f"*_task-{task}_*_eeg.edf")):
        r["INIT"] = "done"

    # PYLOSSLESS — ll_config.yaml under derivatives/pylossless/
    pe = PYLL_DERIV / f"sub-{pid}" / "ses-01" / "eeg"
    if pe.exists() and any(pe.glob(f"*_task-{task}_*_ll_config.yaml")):
        r["PYLOSSLESS"] = "done"
        if r["INIT"] == "pending":
            r["INIT"] = "done"  # PYLS done implies INIT was done

    # SYNC_LOSS — any file under derivatives/sync_loss/
    se = SYNC_DERIV / f"sub-{pid}" / "ses-01" / "eeg"
    if se.exists() and any(se.glob(f"*_task-{task}_*")):
        r["SYNC_LOSS"] = "done"
    ...
    # SEGMENTATION — epoch .fif under derivatives/segment/epoch_fif_files/<task>/
    sd = SEG_DERIV / task
    if sd.exists() and any(sd.glob(f"sub-{pid}_*_task-{task}_*_epo.fif")):
        r["SEGMENTATION"] = "done"

    # AUTOREJECT — epoch .fif under derivatives/autorej/epoch_fif_files/<task>/
    ad = AUTO_DERIV / task
...
def sourcedata_to_bids(raw_name):
    """Convert a raw subject folder name to (bids_pid, hospital)."""
...
def _valid_pid(pid):
    ...
    if "-" in pid:
        return False
    if pid in ("Pilots", "archive", "GO_raw", "__MACOSX", "Q043_F1"):
        return False
...
def scan_participants(progress_cb=None):
    """
    Walk sourcedata and derivatives to build a list of participant dicts.

    DEMOGRAPHIC DATA INTEGRATION NOTE:
    ───────────────────────────────────
    If you want to incorporate an external demographics CSV (e.g. REDCap
    export or a hand-curated spreadsheet), add the merge here, after the
    participant list is assembled.  Example:

        import pandas as pd
        demo_csv = WD / "demographics.csv"   # ← place your file here
        if demo_csv.exists():
            demo_df = pd.read_csv(demo_csv)
            demo_map = demo_df.set_index("q1k_ID").to_dict("index")
            for p in participants:
                extra = demo_map.get(p["id"], {})
                p["age"]   = extra.get("age", "")
                p["group"] = extra.get("group", "")
                p["sex"]   = extra.get("sex", "")

    The tracking_pipeline.py notebook also calls load_redcap() and
    merge_tracking() — to point those at a new file, update the
    'redcap_dir' parameter passed to that notebook's CLI entrypoint.
    """
    pmap = {}
        ...
        for mod in ("eeg", "et"):
            ...
            for rd in sorted(d.iterdir()):
                if not rd.is_dir() or rd.name.startswith("."):
                    continue
                if rd.name in ("Pilots", "archive", "GO_raw", "__MACOSX"):
                    continue
                ...
                if pid not in pmap:
                    pmap[pid] = {
                        "id": pid, "hospital": h,
                        "has_eeg": False, "has_et": False,
                        "raw_name": rd.name, "eeg_tasks": [],
                        "task_stages": {}, "slurm_jobs": {},
                        "notes": "", "et_asc_tasks": [],
                    }
                p = pmap[pid]
                if mod == "eeg":
    ...
    # Also pick up subjects whose BIDS data exists but whose sourcedata
    # folder may have been cleaned up
    for bs in sorted(INIT_DERIV.glob("sub-*/")):
        pid = bs.name[4:]
        if not _valid_pid(pid):
            continue
        ed = bs / "ses-01" / "eeg"
        ...
        if pid not in pmap:
            pmap[pid] = {
                "id": pid, "hospital": hosp,
                "has_eeg": True, "has_et": False,
                "raw_name": raw, "eeg_tasks": [],
                "task_stages": {}, "slurm_jobs": {},
                "notes": "", "et_asc_tasks": [],
            }
        p = pmap[pid]
        ...
        for edf in ed.glob("*_eeg.edf"):
            ...
            if m and m.group(1) in VALID_TASKS:
                tf.add(m.group(1))
        p["eeg_tasks"] = sorted(tf)
...
def stage_counts(participants, task):
    out = {s: {h: defaultdict(int) for h in HOSPITALS + ["ALL"]}
           for s in STAGES}
    for p in participants:
        if task not in p.get("eeg_tasks", []):
            continue
        h  = p["hospital"]
...
def make_slurm_script(task, stage, list_path, site, n, job_name):
    ...
    return (
        ...
        f"{cli} --project-path {PROJECT_PATH} --task {task} "
        f"--subject $SUBJECT {sf} {slf}\n"
    )
...
class App:
    ...
    def __init__(self, scr):
        ...
        self.running = True
        self.tab = 0
        self.msg = "S=scan  1-8=tabs  Q=quit"
        self.msg_ok = True
        self.ps = []
        self.tcounts = {}
        self.task_list = []
        ...
        if self.ps:
            self._rebuild()
            self._status(
                f"Loaded {len(self.ps)} participants — press S to rescan", True
            )
        else:
    ...
    def _rebuild(self):
        """Rebuild task-level counts from the in-memory participant list."""
        self.tcounts = {}
        for p in self.ps:
            for t in p.get("eeg_tasks", []):
                if t not in VALID_TASKS:
                    continue
                if t not in self.tcounts:
                    self.tcounts[t] = defaultdict(int)
                ...
                if all(x == "done" for x in sv):
                    ov = "done"
                elif any(x == "error" for x in sv):
                    ov = "error"
                elif any(x == "done" for x in sv):
                    ov = "partial"
                else:
                    ov = "pending"
                self.tcounts[t][ov] += 1
        ...
        self.task_list = sorted(
            [t for t in self.tcounts if t in VALID_TASKS],
    ...
    def _filt(self):
        if not self.sel_task:
            self.filt_ps = []
            return
        out = []
        for p in self.ps:
            if self.sel_task not in p.get("eeg_tasks", []):
                continue
            if self.fh != "ALL" and p["hospital"] != self.fh:
                continue
            ...
            if self.fss == "PARTIAL" and not (
                any(x == "done" for x in sv) and any(x == "pending" for x in sv)
            ):
                continue
            if self.fsearch and self.fsearch.lower() not in p["id"].lower():
                continue
            out.append(p)
        self.filt_ps = out
        if self.pcur >= len(self.filt_ps):
            self.pcur = max(0, len(self.filt_ps) - 1)
    ...
    def _bar(self, y, x, bw, done, total, sel=False):
        ...
        self._w(y, x + bw,
                ...
                AC() if not sel else SL())

    def _next_pending(self, p, task):
        for s in STAGES:
            if p.get("task_stages", {}).get(task, {}).get(s, "pending") == "pending":
                return s
        return None
    ...
    def _poll_bg(self):
        ...
        try:
            while True:
                ...
                if kind == "done":
                    self.cout.append(
                        (f"  [exit {data}]", "ok" if data == "0" else "err")
                    )
                    self.cout.append(("", "out"))
                    self._bg_done = True
                    if data != "0":
                        self._status(
                            f"Command FAILED (exit {data}) — see Terminal", False
                        )
                    else:
                ...
                if len(self.cout) > 600:
                    self.cout = self.cout[-400:]
                changed = True
        except queue.Empty:
            pass
        return changed
    ...
    def draw(self):
        ...
        h, w = self.scr.getmaxyx()
        if h < 22 or w < 80:
            self._w(0, 0, f"Too small ({w}x{h}) — need 80x22+")
            self.scr.refresh()
            return
        self._draw_hdr(h, w)
        self._draw_tabs(w)
        ...
        cy, ch = 3, h - 4
        fns = [
            self._draw_overview, self._draw_tasks, self._draw_pipeline,
    ...
    def _draw_hdr(self, h, w):
        n = len(self.ps)
        hsj = sum(1 for p in self.ps if p["hospital"] == "HSJ")
        ...
        next_scan = max(0, int(
            AUTO_SCAN_INTERVAL - (time.time() - self._last_scan_time)
        ))
        scan_s = f" [scan in {next_scan}s]" if self._last_scan_time > 0 else ""
        self.scr.attron(HD())
        self._w(0, 0, " " * (w - 1))
        self._w(0, 0, f" Q1K v6{task}{sel_s}{bg_s}{scan_s}")
        self._w(0, max(0, w - 38),
                f" {n} ptcps | HSJ:{hsj} MHC:{n - hsj} | {now} ")
        self.scr.attroff(HD())
    ...
    def _draw_overview(self, y, h, w):
        if not self.ps:
            self._w(y + 2, 4, "No data — press S to scan", YL())
            return
        n = len(self.ps)
        hsj = sum(1 for p in self.ps if p["hospital"] == "HSJ")
        cards = [
        ...
        self._w(y + 4, 1,
                "TASK OVERVIEW — progress = INIT done  ↑↓=select  Enter=Pipeline",
                AC())
        bw = min(28, w - 58)
        self._w(y + 5, 0,
                f"  {'TASK':<10}{'TOTAL':>6}{'INIT':>7}{'PYLS':>7}{'SYNC':>7}{'SEGM':>7}  PROGRESS (INIT)"[:w - 1],
                HD())
        for ri, task in enumerate(self.task_list):
            if y + 6 + ri >= y + h - 1:
                break
            c = self.tcounts.get(task, {})
            tot       = c["total"]
            ...
            sel = task == self.sel_task
            attr = SL() if sel else WH()
            line = (f"  {task:<10}{tot:>6}{init_done:>7}{pyls_done:>7}"
                    f"{sync_done:>7}{segm_done:>7}  ")
    ...
    def _draw_tasks(self, y, h, w):
        self._w(y, 1,
                f"SELECT TASK  Active:[{self.sel_task or 'none'}]  "
                "↑↓=move  Enter=Pipeline", AC())
        bw = min(24, w - 58)
        self._w(y + 1, 0,
                f"  {'#':<4}{'TASK':<12}{'TOTAL':>6}{'INIT':>7}{'PYLS':>7}"
                f"{'SYNC':>7}{'SEGM':>7}  PROGRESS"[:w - 1],
                HD())
        vis = h - 4
        off = max(0, self.tcur - vis // 2)
        for i, task in enumerate(self.task_list[off:off + vis]):
            ...
            sel = idx == self.tcur
            attr = SL() if sel else WH()
            line = (f"  {idx + 1:<4}{task:<12}{tot:>6}{init_done:>7}"
                    f"{pyls_done:>7}{sync_done:>7}{segm_done:>7}  ")
            self._w(row, 0, line[:w - bw - 8], attr)
            self._bar(row, len(line), bw, init_done, tot, sel)
            if sel:
                self._w(row, w - 12, "<- ACTIVE", TB())
    ...
    def _draw_pipeline(self, y, h, w):
        task = self.sel_task
        if not task:
            self._w(y + 2, 4, "No task — press 2", YL())
            return
        if self.pipe_drill:
            self._draw_pipeline_drill(y, h, w)
            return
        ...
        self._w(y, 1,
                f"PIPELINE [{task}]  H:[{self.pipe_hosp}]"
                "  ↑↓=stage  H=hosp  Enter=drill  T=test  Y=submit",
                AC())
        hdr = (f"  {'#':<3}{'STAGE':<14}"
        ...
        for si, stage in enumerate(STAGES):
            ...
            ready = [
                ...
                and (prev is None or
                     p.get("task_stages", {}).get(task, {}).get(prev) == "done")
            ...
            if blocked:
                self._w(r + 1, w // 3 + 14,
                        f"  {len(blocked)} blocked", RD())

        sy = y + 2 + len(STAGES) * 2 + 1
        self._w(sy, 1,
                "Enter=drill  T=test-1-subject  Y=submit-array  "
                "H=toggle-hosp  A=submit-all-stages",
                YL())

    def _draw_pipeline_drill(self, y, h, w):
        task = self.sel_task
        stage = STAGES[self.pipe_stage]
        ps_t = [p for p in self.ps if task in p.get("eeg_tasks", [])]
        ...
        for p in ps_t:
            ...
            if st == "done":
                ...
                cat = "error"
            elif st == "pending" and prev_done and ok:
                cat = "ready"
            elif st == "pending" and (not prev_done or not ok):
                cat = "blocked"
            else:
                cat = "queued"
        ...
        counts = {
            c: sum(1 for _, cat, _ in self.pipe_dlist if cat == c)
            for c in ("done", "ready", "blocked", "error", "queued")
        }
        self._w(y, 1,
                f"DRILL [{task}/{stage}]  H:[{self.pipe_hosp}]  "
                f"done:{counts['done']} ready:{counts['ready']} "
                f"blocked:{counts['blocked']} err:{counts['error']}",
                AC())
        self._w(y, w - 40,
                " Spc=check  B=submit-ready  A=submit-all  ESC=back", YL())

        sab = [STAGE_SHORT[s] for s in STAGES]
        hdr = (f" {'CK':<3}{'ID':<26}{'H':<5}{'CAT':<8}"
               + "".join(f"{s:^7}" for s in sab) + "  INFO")
        self._w(y + 1, 0, hdr[:w - 1], HD())

        vis = h - 5
        if self.pipe_dcur < self.pipe_doff:
            self.pipe_doff = self.pipe_dcur
        if self.pipe_dcur >= self.pipe_doff + vis:
            self.pipe_doff = self.pipe_dcur - vis + 1

        for i, (p, cat, info) in enumerate(
            self.pipe_dlist[self.pipe_doff:self.pipe_doff + vis]
        ):
            row = y + 2 + i
            idx = self.pipe_doff + i
            sel = idx == self.pipe_dcur
            chk = "V" if p["id"] in self.selected_pids else " "
            ts = p.get("task_stages", {}).get(task, {})
            cat_at = (
                GR()  if cat == "done"    else
                OKB() if cat == "ready"   else
                RD()  if cat in ("error", "blocked") else
                MG()
            )
            if sel:
                cat_at = SL()
            ...
            for s in STAGES:
                ...
                self._w(row, x, f" {ic:^5}", at)
                x += 7
            info_s = info[:w - x - 2] if info else ""
            self._w(row, x + 1, info_s, YL() if not sel else SL())

        self._w(y + h - 2, 0,
                " ↑↓=scroll  Space=check  B=submit-ready  "
                "A=submit-all  T=test-1  ESC=back",
                YL())
    ...
    def _draw_participants(self, y, h, w):
        if not self.sel_task:
            ...
            return
        if self.det_p:
            self._draw_detail(y, h, w)
            return
        fbar = (f" Task:[{self.sel_task}] H:[{self.fh}] St:[{self.fss}]"
                f" [{self.fsearch or '-'}] ({len(self.filt_ps)}) "
                f"CHK:{len(self.selected_pids)}")
        self._w(y, 0, fbar[:w - 1], AC())
        self._w(y, w - 34, " H=hosp F=status /=search Spc=check", YL())
        sab = [STAGE_SHORT[s] for s in STAGES]
        hdr = (f" {'CK':<3}{'ID':<26}{'H':<5}"
               + "".join(f"{s:^7}" for s in sab) + "  NEXT")
        self._w(y + 1, 0, hdr[:w - 1], HD())
        vis = h - 5
        if self.pcur < self.poff:
            self.poff = self.pcur
        if self.pcur >= self.poff + vis:
            self.poff = self.pcur - vis + 1
        ...
        if len(self.filt_ps) > vis > 0:
            ...
            for i in range(vis):
                self._w(y + 2 + i, w - 1, "#" if i == sb else "|", AC())
        self._w(y + h - 2, 0,
                " ↑↓ Spc=check Enter=detail R=next-stage T=test "
                "B=batch C=clear H=hosp F=filter /=search",
                YL())
    ...
    def _draw_detail(self, y, h, w):
        ...
        ts = p.get("task_stages", {}).get(task, {})
        # FIX v6: INIT files are under INIT_DERIV, not WD directly
        be = INIT_DERIV / f"sub-{p['id']}" / "ses-01" / "eeg"
        tfiles = list(be.glob(f"*_task-{task}_*_eeg.edf")) if be.exists() else []
        et_ok = p.get("has_et", False)
        asc_tasks = p.get("et_asc_tasks", [])
        self._w(y + 1, 1,
                f"Task:{task}  BIDS-files:{len(tfiles)}  "
                f"ET:{et_ok}  ASC-tasks:{asc_tasks}",
                WH())
        for fi, fn in enumerate(tfiles[:2]):
            self._w(y + 2 + fi, 4, fn.name[:w - 6], WH())
        ...
        for si, stage in enumerate(STAGES):
            ...
            self._w(y + 5 + si, 1,
                    f" {ic} {stage:<14}{st.upper():<10}", bg)
            self._w(y + 5 + si, 27, ver_s[:22], ver_at if not sel else SL())
            self._w(y + 5 + si, 51, job[:18], bg)
            if sel:
                hint = {
                    "pending": "[Enter=submit T=test]",
                    "error":   "[Enter=retry D=debug]",
                    "done":    "done V",
                    "running": "running…",
                }.get(st, "")
        ...
        ay = y + 5 + len(STAGES) + 1
        self._w(ay, 1,
                "R=next-stage  A=all  T=test  D=debug  "
                "L=logs  ↑↓=select  Enter=submit  ESC=back",
                YL())
        ox = w // 2 + 2
        ...
        for ti, t in enumerate(p.get("eeg_tasks", [])):
            ...
            sv  = [tts.get(s, "pending") for s in STAGES]
            dn  = sum(1 for x in sv if x == "done")
            if all(x == "done" for x in sv):
                ta, ic2 = GR(), "DONE"
            elif any(x == "error" for x in sv):
                ta, ic2 = RD(), "ERR "
            else:
                ta, ic2 = WH(), f"{dn}/{len(STAGES)}"
            self._w(y + 2 + ti, ox,
                    f" {'>' if t == task else ' '}{t:<12}{ic2}", ta)
    ...
    def _draw_jobs(self, y, h, w):
        ...
        rw = w - lw - 2
        self._w(y, 1,
                "JOBS  R=refresh  S=rescan+delta  F=errors  "
                "L=list  G=log  D=delta  E=errors",
                AC())
        self._hl(y + 1)
        self._w(y + 2, 0,
                f"{'JOBID':<14}{'NAME':<16}{'STATE':<10}"[:lw], HD())
        vis_j = h - 6
        for i, jd in enumerate(self.jobs_list[:vis_j]):
            ...
            at = (
                SL() if sel else
                GR() if st in ("COMPLETED", "RUNNING") else
                RD() if st in ("FAILED", "CANCELLED", "TIMEOUT") else
                MG() if st == "PENDING" else
                WH()
            )
            self._w(y + 3 + i, 0,
                    f"{jd.get('jobid', ''):<14}"
        ...
        lines = (
            self.jobs_log   if self.jobs_panel == "log"    else
            self.jobs_delta if self.jobs_panel == "delta"  else
            [f"{pid} | {name} | {snip}"
             for pid, name, snip in self.jobs_errors]
            if self.jobs_panel == "errors" else
            self._job_detail_lines()
        )
        sc = self.jobs_log_sc if self.jobs_panel == "log" else 0
        for i, line in enumerate(lines[sc:sc + h - 6]):
            at = (
                RD() if any(k in line for k in
                            ("Error", "ERROR", "Traceback", "FAILED", "Exception"))
                else YL() if "Warning" in line
                else GR() if any(k in line for k in
                                 ("done", "SUCCESS", "completed", "NEW DONE"))
                else WH()
            )
            self._w(y + 3 + i, lw + 1, line[:rw], at)
        self._w(y + h - 1, 0,
                " R=refresh  S=rescan  F=scan-errors  L=list  G=log  "
                "D=delta  E=errors  ↑↓=scroll  Enter=view-log",
                YL())
    ...
    def _job_detail_lines(self):
        ...
        return [
            ...
            "Press E to see error summary",
            "Press S to rescan filesystem",
        ]
    ...
    def _draw_terminal(self, y, h, w):
        ...
        self._w(y, 1,
                f"TERMINAL{bg_ind}  num+Enter=insert  Enter=run  "
                "ESC=leave  ↑↓=history",
        ...
        self._w(y + h - 2, len(pr), self.cin[:mw - len(pr)],
                WH() | curses.A_BOLD)
        try:
            self.scr.move(y + h - 2, min(len(pr) + len(self.cin), mw - 1))
            curses.curs_set(1)
        except Exception:
            pass

        task = self.sel_task or "TASK"
        WDs = str(WD)
        SDs = str(SOURCEDATA)
        SOs = str(SLURM_OUT)
        secs = [
            ...
            ("COUNTS", [
                ("INIT done",  f"ls {WDs}/derivatives/init/ | wc -l"),
                ("PYLS done",  f"ls {WDs}/derivatives/pylossless/ | wc -l"),
                ("SYNC done",  f"ls {WDs}/derivatives/sync_loss/ | wc -l"),
                ("SEGM done",  f"ls {WDs}/derivatives/segment/epoch_fif_files/{task}/ 2>/dev/null | wc -l"),
        ...
        for sname, cmds in secs:
            if row >= y + h - 3:
                break
            self._w(row, ax, f"-{sname}-"[:aw - 1], AC())
            row += 1
            for lbl, cmd in cmds:
                if row >= y + h - 3:
                    break
                num = len(self._acmds) + 1
                self._acmds.append(cmd)
                self._w(row, ax, f"{num:>2}:{lbl}"[:aw - 1], YL())
                row += 1
    ...
    def _draw_debug(self, y, h, w):
        self._w(y, 1,
                "DEBUG  R=failed-jobs  L=load-log  F=scan-errors  "
                "Tab=run-fix  ↑↓=scroll",
                AC())
        self._hl(y + 1)
        lw = 36
        rw = w - lw - 2
        self._w(y + 2, 0,
                f"{'JOBID':<14}{'NAME':<14}{'STATE':<8}"[:lw], HD())
        for i, (jid, name, state) in enumerate(self.dbg_jobs[:h - 6]):
            sel = i == self.dbg_cur
            at = (
                SL() if sel else
                GR() if state == "COMPLETED" else
                RD() if state in ("FAILED", "CANCELLED", "TIMEOUT") else
                YL() if state == "RUNNING" else
                WH()
            )
            self._w(y + 3 + i, 0,
                    f"{jid:<14}{name[:13]:<14}{state:<8}"[:lw], at)
        ...
        if sug:
            sy = y + h - len(sug) - 2
            self._w(sy - 1, lw + 1,
                    "── FIX SUGGESTIONS (Tab=run first) ──", YL())
            for i, (desc, _) in enumerate(sug):
                self._w(sy + i, lw + 1, f"  {i + 1}. {desc}"[:rw], YL())
        self._w(y + h - 1, 0,
                " R=failed-jobs  L=load-log  F=scan-errors  "
                "Tab=run-fix  ↑↓=navigate  Enter=load",
                YL())
    ...
    def _fix_suggestions(self):
        ...
        if "ModuleNotFoundError" in text or "ImportError" in text:
            ...
            s.append((
                f"Missing module '{m.group(1) if m else '?'}' — activate venv",
                f"source {VENV}",
            ))
        ...
        if "edf2ascii" in text or (
            "ascii" in text.lower() and "not found" in text.lower()
        ):
            s.append(("Missing edf2ascii — need EyeLink module",
                      "module load eyelink"))
        if "/tmp/" in text:
            s.append(("Script reads /tmp/ — must use lustre path!",
                      "# Edit script to use /lustre07/..."))
        if "DIN2" in text and "KeyError" in text:
            s.append(("GO/RS DIN KeyError — check NO_DIN_OFFSET_TASKS in config",
                      ""))
        return s[:5]
    ...
    def _draw_help(self, y, h, w):
        lines = [
            ("Q1K Pipeline Manager v6", AC()),
            ("", WH()),
            ("FIXES IN v6:", HD()),
            ("  Derivative paths corrected to match what the notebooks write:", WH()),
            ("    INIT    → derivatives/init/sub-<pid>/ses-01/eeg/", WH()),
            ("    PYLS    → derivatives/pylossless/sub-<pid>/ses-01/eeg/", WH()),
            ("    SYNC    → derivatives/sync_loss/sub-<pid>/ses-01/eeg/", WH()),
            ("    SEGM    → derivatives/segment/epoch_fif_files/<task>/", WH()),
            ("    AUTO    → derivatives/autorej/epoch_fif_files/<task>/", WH()),
            ("  detect_stage() / verify_data() updated accordingly.", WH()),
            ("  scan_participants() picks up INIT-derived BIDS files correctly.", WH()),
            ("", WH()),
            ("DEMOGRAPHIC DATA:", HD()),
            ("  scan_participants() has a clearly-marked block at the end where", WH()),
            ("  you can merge an external CSV (demographics.csv) into each", WH()),
            ("  participant dict.  See the docstring in that function.", WH()),
            ("  tracking_pipeline.py uses load_redcap(redcap_dir) — pass your", WH()),
            ("  REDCap export directory via the --redcap-dir CLI flag.", WH()),
            ("", WH()),
            ("TABS:", HD()),
    ...
    def _h_overview(self, key):
        if key == curses.KEY_UP:
            i = (self.task_list.index(self.sel_task)
                 if self.sel_task in self.task_list else 0)
            self.sel_task = self.task_list[max(0, i - 1)]
            self._filt()
        elif key == curses.KEY_DOWN:
            i = (self.task_list.index(self.sel_task)
                 if self.sel_task in self.task_list else 0)
            self.sel_task = self.task_list[min(len(self.task_list) - 1, i + 1)]
            self._filt()
        elif key == 10:
            self.tab = 2
    ...
    def _h_tasks(self, key):
        if key == curses.KEY_UP:
            ...
            self.tcur = min(len(self.task_list) - 1, self.tcur + 1)
        elif key in (10, curses.KEY_RIGHT):
            if self.task_list:
                self.sel_task = self.task_list[self.tcur]
                self._filt()
                self.tab = 2
    ...
    def _h_pipeline(self, key):
        if self.pipe_drill:
            if key == 27:
                ...
                self.pipe_dcur = max(0, self.pipe_dcur - 1)
            elif key == curses.KEY_DOWN:
                self.pipe_dcur = min(len(self.pipe_dlist) - 1,
                                     self.pipe_dcur + 1)
            elif key == ord(" "):
                if self.pipe_dlist:
                    ...
                    self.pipe_dcur = min(self.pipe_dcur + 1,
                                        len(self.pipe_dlist) - 1)
            elif key in (ord("b"), ord("B")):
                self._batch_submit_selected()
            elif key in (ord("a"), ord("A")):
                self._submit_all_ready(self.sel_task, STAGES[self.pipe_stage])
            elif key in (ord("t"), ord("T")):
                self._test_run_bg(self.sel_task, STAGES[self.pipe_stage])
            return
        if key == curses.KEY_UP:
            self.pipe_stage = max(0, self.pipe_stage - 1)
        elif key == curses.KEY_DOWN:
            self.pipe_stage = min(len(STAGES) - 1, self.pipe_stage + 1)
        elif key == 10:
            self.pipe_drill = True
            self.pipe_dcur = 0
            self.pipe_doff = 0
        elif key in (ord("h"), ord("H")):
            opts = ["ALL"] + HOSPITALS
            self.pipe_hosp = opts[(opts.index(self.pipe_hosp) + 1) % len(opts)]
        elif key in (ord("t"), ord("T")):
            self._test_run_bg(self.sel_task, STAGES[self.pipe_stage])
            ...
            if self.sel_task:
                for s in STAGES:
                    self._submit_all_ready(self.sel_task, s)
    ...
    def _h_participants(self, key):
        if self.det_p:
            if key in (27, ord("q")):
                ...
                self.det_sc = max(0, self.det_sc - 1)
            elif key == curses.KEY_DOWN:
                self.det_sc = min(len(STAGES) - 1, self.det_sc + 1)
            elif key == 10:
                self._sub_p_stage(self.det_p, self.sel_task,
                                  STAGES[self.det_sc])
            elif key in (ord("r"), ord("R")):
                self._next_stage_p(self.det_p, self.sel_task)
            elif key in (ord("a"), ord("A")):
                self._all_stages_p(self.det_p, self.sel_task)
            elif key in (ord("t"), ord("T")):
                self._test_run_bg(self.sel_task, STAGES[self.det_sc],
                                  self.det_p)
            elif key in (ord("d"), ord("D"), ord("l"), ord("L")):
                self._load_dbg_participant(self.det_p)
                self.tab = 6
            return
        if key == curses.KEY_UP:
            self.pcur = max(0, self.pcur - 1)
        elif key == curses.KEY_DOWN:
            self.pcur = min(len(self.filt_ps) - 1, self.pcur + 1)
        elif key == curses.KEY_PPAGE:
            self.pcur = max(0, self.pcur - 10)
        elif key == curses.KEY_NPAGE:
            self.pcur = min(len(self.filt_ps) - 1, self.pcur + 10)
        elif key == ord(" "):
            if self.filt_ps:
            ...
            if self.filt_ps:
                self.det_p = self.filt_ps[self.pcur]
                self.det_sc = 0
        elif key in (ord("r"), ord("R")):
            if self.filt_ps:
                self._next_stage_p(self.filt_ps[self.pcur], self.sel_task)
        elif key in (ord("t"), ord("T")):
            if self.filt_ps:
            ...
            self._status("Cleared", True)
        elif key in (ord("h"), ord("H")):
            opts = ["ALL", "HSJ", "MHC"]
            self.fh = opts[(opts.index(self.fh) + 1) % 3]
            self._filt()
        elif key in (ord("f"), ord("F")):
            opts = ["ALL", "DONE", "ERROR", "PENDING", "PARTIAL"]
            self.fss = opts[(opts.index(self.fss) + 1) % len(opts)]
            self._filt()
        elif key == ord("/"):
            self._do_search()
        elif key == 27:
    ...
    def _h_jobs(self, key):
        if key in (ord("r"), ord("R")):
            ...
            self.jobs_panel = "delta"
        elif key in (ord("e"), ord("E")):
            self.jobs_panel = "errors"
        elif key == curses.KEY_UP:
            if self.jobs_panel == "log":
                self.jobs_log_sc = max(0, self.jobs_log_sc - 1)
            else:
                self.jobs_cur = max(0, self.jobs_cur - 1)
        elif key == curses.KEY_DOWN:
            if self.jobs_panel == "log":
                self.jobs_log_sc += 1
            else:
                self.jobs_cur = min(len(self.jobs_list) - 1, self.jobs_cur + 1)
        elif key == 10:
            if self.jobs_list and self.jobs_cur < len(self.jobs_list):
                self._load_job_log(self.jobs_list[self.jobs_cur])
                self.jobs_panel = "log"
    ...
    def _h_debug(self, key):
        if key in (ord("r"), ord("R")):
            ...
            if self.dbg_jobs and self.dbg_cur < len(self.dbg_jobs):
                jid, name, _ = self.dbg_jobs[self.dbg_cur]
                self._load_slurm_log(jid, name)
        elif key in (ord("f"), ord("F")):
            self._scan_all_errors()
        elif key == curses.KEY_UP:
            if self.dbg_sc > 0:
                self.dbg_sc -= 1
            elif self.dbg_cur > 0:
            ...
            if self.dbg_sc < len(self.dbg_lines) - 1:
                self.dbg_sc += 1
            ...
            if sug:
                ...
                if cmd:
                    self.tab = 5
                    self.cin = cmd
                    self._status("Fix loaded in Terminal — press Enter", True)
    ...
    def _h_terminal(self, key):
        ...
        if key == 10:
            ...
            if self.chidx > 0:
                self.chidx -= 1
                self.cin = self.chist[self.chidx]
        elif key == curses.KEY_DOWN:
            if self.chidx < len(self.chist) - 1:
                self.chidx += 1
                self.cin = self.chist[self.chidx]
            else:
                self.chidx = len(self.chist)
                self.cin = ""
        elif key == 9:
            self._autocomplete()
    ...
    def _confirm(self, lines):
        ...
        bw = min(max((len(l) for l in lines), default=40) + 6, w - 4)
        by = (h - bh) // 2
        bx = (w - bw) // 2
        try:
            for row in range(bh):
                self._w(by + row, bx, " " * bw, HD())
            self._w(by,          bx, "┌" + "─" * (bw - 2) + "┐", AC())
            self._w(by + bh - 1, bx, "└" + "─" * (bw - 2) + "┘", AC())
            for row in range(1, bh - 1):
                self._w(by + row, bx, "│", AC())
                self._w(by + row, bx + bw - 1, "│", AC())
            self._w(by + 1,      bx + 2, "CONFIRM SUBMISSION", HD())
            self._w(by + 2,      bx + 2, "─" * (bw - 4), AC())
            for i, line in enumerate(lines[:bh - 6]):
                self._w(by + 3 + i, bx + 2, line[:bw - 4], WH())
    ...
    def _do_scan(self, silent=False):
        h, w = self.scr.getmaxyx()
        if not silent:
            self._status("Scanning filesystem…", True)
            self.draw()
            self.scr.refresh()
        ...
        def cb(i, total, pid):
            if not silent and i % 20 == 0:
                ...
                self._w(h // 2, 2,
                        f"  Scanning {i}/{total} ({pct:.0f}%) — {pid:<30}  ",
                        AC())
                self.scr.refresh()
        ...
        try:
            old_done = {
                p["id"]: {
                    t: {
                        s for s, v in p.get("task_stages", {}).get(t, {}).items()
                        if v == "done"
                    }
                    for t in p.get("eeg_tasks", [])
                }
            ...
            hsj = sum(1 for p in self.ps if p["hospital"] == "HSJ")
            self._status(
                f"Scanned {n} | HSJ:{hsj} MHC:{n - hsj} | "
                f"{len(self.task_list)} tasks | {len(delta)} new completions",
                True,
            )
        except Exception as e:
            import traceback
            self._status(f"Scan error: {e}", False)
    ...
    def _test_run_bg(self, task, stage, p=None):
        ...
        if not ps_t:
            self._status(
                f"No verified-ready subjects for {task}/{stage}", False
            )
            return
        subject = ps_t[0]
        pid = subject["id"]
        site = SITE_FLAG.get(subject["hospital"], subject["hospital"])
        cli = STAGE_CLI[stage]
        subj_arg = subject.get("raw_name", pid)
        flags = (f"--project-path {PROJECT_PATH} --task {task} "
                 f"--subject {subj_arg}")
        if stage == "INIT":
            flags += f" --site {site}"
        ...
        if not ok:
            ...
            self.dbg_lines = [
                f"Data verification failed for {pid}/{task}/{stage}",
                "", f"Reason: {reason}",
            ]
            return
    ...
    def _submit_all_ready(self, task, stage):
        ...
        if not ready:
            self._status(
                f"No verified-ready candidates for {task}/{stage}", False
            )
            return
        for site in HOSPITALS:
            sp = [p for p in ready if p["hospital"] == site]
            if sp:
                self._submit_array_for(task, stage, site, sp)

    def _submit_array_for(self, task, stage, site, ps_list):
        n = len(ps_list)
        jname = f"{task}_{STAGE_SHORT[stage]}_{site}"
        cli = STAGE_CLI[stage]
        sf = f"--site {SITE_FLAG.get(site, site)}" if stage == "INIT" else ""
        preview = ps_list[:8]
        blocked = [
        ...
        if not self._confirm(
            ...
            self._status(f"Cancelled {task}/{stage}/{site}", True)
            return
        ...
        try:
            ...
            sp.write_text(make_slurm_script(task, stage, lp, site, n, jname))
            sp.chmod(0o755)
            self._status(
                f"Submitting {n} jobs {task}/{stage}/{site}…", True
            )
            self.draw()
            self.scr.refresh()
            ...
            if code == 0:
                ...
                for p in ps_list:
                    p.setdefault("slurm_jobs", {})[f"{task}:{stage}"] = (
                        f"{jid}_arr"
                    )
                save_inv(self.ps)
                self._rebuild()
                self._status(
                    f"Submitted {n} | {task}/{stage}/{site} | JobArray:{jid}",
                    True,
                )
            else:
                self._status(f"sbatch failed: {err[:80]}", False)
                self.dbg_lines = [
                    f"sbatch error {task}/{stage}/{site}:", "", err
                ]
                self.dbg_sc = 0
        except Exception as e:
            self._status(f"Submit error: {e}", False)

    def _sub_p_stage(self, p, task, stage):
        ts = p.get("task_stages", {}).get(task, {})
        if ts.get(stage, "pending") not in ("pending", "error"):
            self._status(f"{stage} is {ts.get(stage)}", False)
            return
        ok, reason = verify_data(p, task, stage)
        if not ok:
            self._status(f"Data check failed: {reason}", False)
            self.dbg_lines = [
                f"Cannot submit {p['id']}/{task}/{stage}", "", f"Reason: {reason}"
            ]
            return
        cli = STAGE_CLI[stage]
        site = SITE_FLAG.get(p["hospital"], p["hospital"])
        subj_arg = p.get("raw_name", p["id"])
        flags = (f"--project-path {PROJECT_PATH} --task {task} "
                 f"--subject {subj_arg}")
        if stage == "INIT":
            flags += f" --site {site}"
        if stage in ("PYLOSSLESS", "AUTOREJECT"):
            flags += " --slurm"
        if not self._confirm([
            f"  {p['id']}  ({p['hospital']})",
            f"  {task} / {stage}",
            ...
            return
        cmd = venv_cmd(f"{cli} {flags}")
        self._status(f"Submitting {p['id']}/{task}/{stage}…", True)
        self.draw()
        self.scr.refresh()
        out, err, code = run_cmd(cmd, 60)
        if code == 0:
            m = re.search(r"(\d+)", out)
            jid = m.group(1) if m else "direct"
            p.setdefault("task_stages", {}).setdefault(
                task, {s: "pending" for s in STAGES}
            )[stage] = "queued"
            p.setdefault("slurm_jobs", {})[f"{task}:{stage}"] = jid
            save_inv(self.ps)
            self._rebuild()
            self._status(
                f"Submitted {p['id']}/{task}/{stage} Job:{jid}", True
            )
        else:
            self._status(f"Failed: {err[:80]}", False)
            self.dbg_lines = [
                f"Error: {p['id']}/{task}/{stage}", "", out, err
            ]
            self.dbg_sc = 0
    ...
    def _next_stage_p(self, p, task):
        ...
        if s:
            self._sub_p_stage(p, task, s)
        else:
            self._status(f"{p['id']}/{task}: nothing pending", True)

    def _all_stages_p(self, p, task):
        count = 0
        for s in STAGES:
            if p.get("task_stages", {}).get(task, {}).get(s, "pending") in (
                "pending", "error"
            ):
                self._sub_p_stage(p, task, s)
                count += 1
        if not count:
            self._status(f"Nothing to submit for {p['id']}/{task}", True)

    def _batch_submit_selected(self):
        if not self.selected_pids:
            self._status("No participants checked", False)
            return
        if not self.sel_task:
            self._status("No task", False)
            return
        task = self.sel_task
        sel_list = [
            p for p in self.ps
            if p["id"] in self.selected_pids
            and task in p.get("eeg_tasks", [])
        ]
        by_stage = defaultdict(list)
        for p in sel_list:
            s = self._next_pending(p, task)
            if s and verify_data(p, task, s)[0]:
                by_stage[s].append(p)
        if not by_stage:
            self._status("No verified-ready subjects in selection", False)
            return
    ...
    def _load_job_log(self, jd):
        ...
        if SLURM_OUT.exists():
            found = (
                list(SLURM_OUT.glob(f"*_{jid}_*.out"))
                + list(SLURM_OUT.glob(f"*_{jid}.out"))
                + list(SLURM_OUT.glob(f"{name}*{jid}*.out"))
            )
        found += (list(WD.glob(f"slurm-{jid}.out"))
                  + list(WD.glob(f"slurm-{jid}_*.out")))
        if not found:
            self.jobs_log += [
                f"No log found for {jid}",
            ...
            try:
                self.jobs_log += lf.read_text(errors="replace").split("\n")
            except Exception as e:
                self.jobs_log += [f"Read error: {e}"]
        self.jobs_log_sc = 0

    def _scan_errors(self):
        self._status("Scanning logs…", True)
        self.draw()
        self.scr.refresh()
        self.jobs_errors = []
        self.jobs_log = ["=== ERROR SCAN ===", ""]
        count = 0
        if SLURM_OUT.exists():
            for lf in sorted(SLURM_OUT.glob("*.out"))[-50:]:
                try:
                    ...
                    pass
        if count == 0:
            self.jobs_log += ["No errors found in recent logs — looking good!"]
        self.jobs_log_sc = 0
        self.jobs_panel = "errors"
    ...
    def _load_failed_jobs(self):
        ...
        try:
            ...
            for line in r.stdout.strip().split("\n"):
                ...
                if (len(parts) >= 3 and "." not in parts[0]
                        and parts[0].strip()):
                    self.dbg_jobs.append((
                        parts[0].strip(),
                        parts[1].strip() if len(parts) > 1 else "?",
                        parts[2].strip() if len(parts) > 2 else "?",
                    ))
            if not self.dbg_jobs:
                self.dbg_lines = ["No failed jobs today!"]
                self.dbg_sc = 0
            else:
                self.dbg_lines = [
                    f"Found {len(self.dbg_jobs)} failed jobs",
                    "↑↓ to select, L to load log",
                ]
                self.dbg_cur = 0
                jid, name, _ = self.dbg_jobs[0]
                self._load_slurm_log(jid, name)
            self._status(f"Loaded {len(self.dbg_jobs)} failed jobs", True)
        except Exception as e:
            self._status(f"sacct error: {e}", False)
    ...
    def _load_slurm_log(self, jid, name):
        ...
        if not found:
            ...
            lf = sorted(set(found))[-1]
            try:
                self.dbg_lines += lf.read_text(errors="replace").split("\n")
            except Exception as e:
                self.dbg_lines += [f"Read error: {e}"]
        self.dbg_sc = 0

    def _load_dbg_participant(self, p):
        self.dbg_lines = [f"=== Logs for {p['id']} ===", ""]
        found = (
            list(SLURM_OUT.glob(f"*{p['id']}*.out"))
            if SLURM_OUT.exists() else []
        )
        if not found:
            for key, jid in p.get("slurm_jobs", {}).items():
                found.extend(
                    SLURM_OUT.glob(f"*_{jid.split('_')[0]}*.out")
                    if SLURM_OUT.exists() else []
                )
        if not found:
            self.dbg_lines += ["No log files found"]
        else:
            lf = sorted(set(found))[-1]
            try:
                self.dbg_lines += (
                    [f"File: {lf.name}", ""]
                    + lf.read_text(errors="replace").split("\n")
                )
            except Exception as e:
                self.dbg_lines += [f"Error: {e}"]
        self.dbg_sc = 0
        self.dbg_jobs = [
            (jid, key, "?") for key, jid in p.get("slurm_jobs", {}).items()
        ]
        self.dbg_cur = 0

    def _scan_all_errors(self):
        self._status("Scanning…", True)
        self.draw()
        self.scr.refresh()
        self.dbg_lines = ["=== ERROR SCAN ===", ""]
        count = 0
        if SLURM_OUT.exists():
            for lf in sorted(SLURM_OUT.glob("*.out"))[-50:]:
                try:
                    ...
                    pass
        if count == 0:
            self.dbg_lines += ["No errors found."]
        self.dbg_sc = 0
        self._status(f"Errors in {count} files", count > 0)
    ...
    def _exec(self):
        ...
        if cmd.isdigit():
            idx = int(cmd) - 1
            if 0 <= idx < len(self._acmds):
                self.cin = self._acmds[idx]
                return
        self.chist.append(cmd)
        self.chidx = len(self.chist)
        self.cin = ""
        ...
        if cmd.startswith("cd "):
            try:
                ...
                if not nw.is_absolute():
                    nw = self.cwd / nw
                nw = nw.resolve()
                if nw.exists():
                    self.cwd = nw
                    os.chdir(nw)
                    self.cout.append((f"  -> {self.cwd}", "ok"))
                else:
                    self.cout.append((f"  Not found: {nw}", "err"))
            except Exception as e:
                self.cout.append((f"  {e}", "err"))
            return
        if not self._bg_done:
            self._status(
                "Background job still running — wait or open new terminal",
                False,
            )
            return
        self._bg_cmd = cmd
...
if __name__ == "__main__":
    try:
        ...
        pass
    print("\nQ1K Pipeline Manager v6 closed.")
