...
def parameters():
    ...
    participants_tsv = "/lustre07/scratch/rsweety/white_paper/wd/derivatives/init/VEP/participants.tsv"
    recompute = True
...
def setup(np, Path):
	...
	data_out = Path("/lustre07/scratch/rsweety/white_paper/wd/derivatives/Analysis/VEP/VEP_xr_itc")
...
def compute_tfr(
    ...
    if recompute:
        ...
        epoch_files = sorted(
            Path("/lustre07/scratch/rsweety/white_paper/wd/derivatives/autoreject/epoch_fif_files/VEP").glob("*_epo.fif")
        )
        for filepath in tqdm(list(epoch_files)):
            ...
            for condition in conditions:
                ...
                }).to_netcdf(path_out, engine="scipy")
...
def load_data(np, xr, pd, load_participants_tsv, participants_tsv, data_out):
    ...
    if not nc_files:
        ...
        dataset = xr.open_mfdataset(str(data_out / "*.nc"), data_vars="minimal",  engine="scipy",)
...
def plot_itc(
    ...
    if dataset is None:
        ...
        data_sites = {
            ...
            "MHC": dataset[kind].sel(participant=dataset["sites"] == "MHC"),
        }
        ...
        for ax, cond in zip(ax_col[:2], data_xr.condition):
            ...
            mask_significance(
                [
                    ...
                    data_sites["MHC"].sel(condition=cond)
                    .transpose("participant", "freqs", "times").values,
                ],
        ...
        for ax, factor, vmin, vmax in zip(
            ...
            contrast_mni = (
                data_sites["MHC"].sel(condition="sv06")
                + factor * data_sites["MHC"].sel(condition="sv15")
            )
            if factor == 1:
                contrast_hsj /= 2
                contrast_mni /= 2
...
def save_figure(fig):
    ...
    if fig is not None:
        fig.savefig(
            "/lustre07/scratch/rsweety/white_paper/wd/derivatives/Analysis/VEP/VEP.png",
            dpi=300,
        )
        print("Saved VEP.png")
    return ()
...
def sample_sizes(np, dataset):
    ...
    if dataset is not None and "sites" in dataset:
        ...
        n_mhc = int(np.sum(dataset["sites"] == "MHC"))
        n_nim = int(np.sum(dataset["sites"] == "NIM"))
        print(f"Sample sizes: HSJ={n_hsj}; MHC={n_mhc}; NIM={n_nim}")
    return ()
