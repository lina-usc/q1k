...
def header(subject_id, task_id):
    import marimo as mo
    mo.md(
        f"# Single Subject Segmentation Q1K — Resting State\n\n"
...
def load_data(mne, mne_bids, project_path, subject_id, session_id,
    ...
    # FIX: sync_loss output is written directly to derivatives/sync_loss,
    # not nested under derivatives/pylossless/derivatives/sync_loss.
    # Using the flat path that matches save_output in sync_loss_report.py.
...
def detect_task(eeg_event_dict, _detect_rsrio, task_id):
    """Auto-detect whether this recording is plain RS or RSRio."""
...
def create_epochs(segment_resting_state, segment_rsrio,
                  eeg_raw, eeg_events, eeg_event_dict, actual_task):
...
def save_epochs(epochs, bids_path, project_path, derivative_base,
                actual_task, pp):
    # FIX: write segment output to derivatives/segment, mirroring the flat
    # structure used by the GO and VEP segment scripts.
...
def plot_psd(epochs, np, FRONTAL_ROI):
    ...
    fig2 = epo_power.plot(picks=FRONTAL_ROI, exclude="bads", amplitude=False)
