...
def parameters():
    ...
    participants_tsv = "/lustre07/scratch/rsweety/white_paper/wd/derivatives/init/PLR/participants.tsv"
...
@app.cell
def load_epochs(Path, task):
    """Get AutoReject epoch file paths for PLR task."""
    epoch_dir = Path(
        f"/lustre07/scratch/rsweety/white_paper/wd/derivatives/autoreject/epoch_fif_files/{task}"
    )
    epoch_paths = sorted(epoch_dir.glob("*_epo.fif"))
    print(f"Found {len(epoch_paths)} AutoReject epoch files for {task}")
    return (epoch_paths,)
...
def build_figure(
    ...
    if not epoch_paths:
        ...
        def plot_epochs(epochs, axes, tmin=-1, tmax=2.5):
            pupil_ch = 'pupil_left' if 'pupil_left' in epochs.ch_names else 'pupil_right'
            
            axes[0].plot(
                epochs.times,
                epochs.get_data(pupil_ch).squeeze().T * 1e3,
                alpha=0.1, color="k",
            )
            axes[0].plot(
                epochs.times,
                epochs.get_data(pupil_ch).squeeze().T.mean(axis=1) * 1e3,
                color="r",
            )
        ...
        for path in tqdm(epoch_paths):
            ...
            epochs_temp = mne.read_epochs(path)
            if "plro_d" not in epochs_temp.event_id:
                continue
            epochs_et = epochs_temp["plro_d"]
            ...
            pupil_ch = "pupil_left" if "pupil_left" in epochs_et.ch_names else "pupil_right"
            plr_sigs.append(epochs_et.get_data(pupil_ch).squeeze().T.mean(axis=1))

            #plr_sigs.append(np.atleast_2d(epochs_et.get_data(pupil_ch).squeeze()).mean(axis=1))
            ax_row[0].plot(
            ...
            e70_data = epochs_eeg.get_data(test_ch)
...
def save_figure(fig):
    """Save the PLR figure."""
    if fig is not None:
        fig.savefig("/lustre07/scratch/rsweety/white_paper/wd/derivatives/Analysis/PLR/PLR_Analysis.png"
                    , dpi=300,
                    bbox_inches="tight", 
                    pad_inches=0)
        print("Saved PLR_Analysis.png")
    return ()
...
def site_summary(pd, load_participants_tsv, participants_tsv, subjects):
    ...
    if participants_tsv and subjects:
        ...
        subjects_clean = pd.Series(subjects, dtype=str).str.strip()
        summary_df = pd.DataFrame({"participant_id": subjects_clean})
        summary_df["site"] = summary_df["participant_id"].str.extract(
            r"^(HSJ|MHC|NIM)", expand=False
        )

        site_counts = summary_df["site"].value_counts().sort_index()
