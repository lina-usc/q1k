    ...
    if not epoch_paths:
        print("No epoch files found.")
            ...
            def plot_epochs(epochs, axes, tmin=-1, tmax=2.5):
                pupil_ch = [ch for ch in epochs.ch_names if 'pupil' in ch.lower()][0]
                axes[0].plot(
                    epochs.times,
                    #epochs.get_data("pupil_left").squeeze().T * 1e3,
                    #epochs.get_data([ch for ch in epochs.ch_names if 'pupil' in ch.lower()][0]).squeeze().T * 1e3,
                    epochs.get_data(pupil_ch).squeeze(axis=1).T * 1e3,
                    alpha=0.1, color="k",
                )
                axes[0].plot(
                    epochs.times,
                    #epochs.get_data("pupil_left").squeeze().T.mean(axis=1) * 1e3,
                    #epochs.get_data([ch for ch in epochs.ch_names if 'pupil' in ch.lower()][0]).squeeze().T.mean(axis=0) * 1e3,
                    epochs.get_data(pupil_ch).squeeze(axis=1).T.mean(axis=1) * 1e3,
                    color="r",
                )
                #pupil_data = epochs.get_data(pupil_ch).squeeze(axis=1).T  # Shape: (times, epochs)
                # Baseline correct: subtract mean of pre-stimulus period (-0.5 to 0)
                #baseline_mask = (epochs.times >= -0.5) & (epochs.times < 0)
                #pupil_data = pupil_data - pupil_data[baseline_mask, :].mean(axis=0)
                ...
                axes[1].plot(
                    ...
                    epochs.get_data(test_ch).squeeze(axis=1).T * 1e6,
                    alpha=0.1, color="k",
                )
                axes[1].plot(
                    epochs.times,
                    #epochs.get_data(test_ch).squeeze().T.mean(axis=1) * 1e6,
                    epochs.get_data(test_ch).squeeze(axis=1).T.mean(axis=1)*1e6,
                    color="r",
                )
            ...
            for path in epoch_paths:
                ...
                if "plro" in epochs.event_id:
                    epochs_filtered = epochs["plro"]
                    if len(epochs_filtered) > 0:
                        example_epoch = epochs_filtered
            ...
            for path in tqdm(epoch_paths):
                subject = path.name.split("/")[-1].split("_")[0][4:]
                epochs_et = mne.read_epochs(path)
                # Skip if plro event not present
                if "plro" not in epochs_et.event_id:
                    continue
                epochs_et = epochs_et["plro"]
                if len(epochs_et) == 0:
                    continue

                pupil_candidates = [ch for ch in epochs_et.ch_names if 'pupil' in ch.lower()]
                if not pupil_candidates:
                    continue
                #pupil_ch = [ch for ch in epochs_et.ch_names if 'pupil' in ch.lower()][0]
                pupil_ch = pupil_candidates[0]
                if pupil_ch:
                    plr_sigs.append(epochs_et.get_data(pupil_ch).squeeze(axis=1).T.mean(axis=1))
                    #pupil_data = epochs_et.get_data(pupil_ch).squeeze(axis=1).T
                    #baseline_mask = (epochs_et.times >= -0.5) & (epochs_et.times < 0)
                    #pupil_baseline_corrected = pupil_data - pupil_data[baseline_mask, :].mean(axis=0)
                    #plr_sigs.append(pupil_baseline_corrected.mean(axis=1))
                else:
                    print(f"Warning: No pupil channel found for subject, skipping")
                    continue
                #plr_sigs.append(epochs_et.get_data(pupil_ch).squeeze().T.mean(axis=1))
                ...
                epochs_eeg = mne.read_epochs(path)
                if "plro" not in epochs_eeg.event_id:
                    continue
                epochs_eeg = epochs_eeg["plro"]
                ch_names = [ch for ch in epochs_et.ch_names if ch[0] == "E"]
                epochs_eeg = epochs_eeg.pick(ch_names)

                erp_sigs.append(epochs_eeg.get_data(test_ch).squeeze(axis=1).T.mean(axis=1))
